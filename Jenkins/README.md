...
**폴링**으로 감지해 앱(frontend+backend)을 빌드(의존성은 Nexus)하고, 이미지를 **ECR** push 후
**GitOps repo 의 이미지 태그를 커밋**합니다. **배포(EKS)는 ArgoCD(CD)** 가 담당 → [`gitops/`](../gitops/README.md).
**GUI 미사용**(JCasC/CLI), **AWS CLI 미사용**(ECR 인증만 REST/SigV4).
...
Nexus(<NEXUS_IP>) ──npm(cicd→nexus:443)────────▶│  frontend/·backend/ 각각 docker build
                                                   │  push ─▶ ECR (REST/SigV4, VPC 엔드포인트)
                                                   └  이미지 태그 커밋 ─▶ Gitea(gitops repo)
                                                                            └─watch─▶ ArgoCD ─▶ EKS
```
...
| 이미지 | `jenkins/jenkins:2.479.2-lts-jdk17` + `nginx:1.27-alpine` |
| 관리자 | `admin` / `.env` 의 `JENKINS_ADMIN_PASSWORD` (웹 UI 미사용) |
| 트리거 | **폴링(pollSCM 3분)** — Jenkins 인바운드 0 |
| 파이프라인 | Node.js(npm→Nexus) → 컨테이너 → **ECR push** → **GitOps 태그 커밋** (배포는 ArgoCD) |
...
| (상세) CI 파이프라인 | [pipeline/README.md](pipeline/README.md) | Jenkinsfile·ECR·SigV4·GitOps 커밋·SG 규칙 |
