#!/usr/bin/env bash
# [폐쇄망 VM] 에서 실행합니다.
# 브라우저 설치 마법사 없이 REST API(curl)만으로 admin 비밀번호 변경 및
# 익명 접근 비활성화를 수행합니다. 03-start.sh 로 Nexus가 기동된 뒤 실행하세요.
set -euo pipefail
cd "$(dirname "$0")/.."
...
curl -sf -u "admin:${CURRENT_PW}" -X PUT \
...
if [ "${DISABLE_ANONYMOUS_ACCESS}" = "true" ]; then
  ...
  curl -sf -u "admin:${ADMIN_PASSWORD}" -X PUT \
...
echo ">> Nexus CLI 초기 설정 완료."
echo "   Web (선택): http://<VM_IP>:${HTTP_PORT}"
echo "   관리자 계정: admin / (.env 의 ADMIN_PASSWORD)"
echo ""
echo ">> 이후 저장소(repository) 생성 등도 REST API로 UI 없이 진행할 수 있습니다. 예:"
echo "   curl -u admin:<password> -X GET ${BASE_URL}/service/rest/v1/repositories"
