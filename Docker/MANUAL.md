...
`x86_64` 가 나오면 `.env` 의 `ARCH=x86_64` 와 일치해야 합니다. `aarch64` 라면 로컬 PC에서
`.env` 의 `ARCH=aarch64` 로 바꿔 바이너리를 다시 받아 전송해야 합니다 (VM에서 값만 바꿔서는
동작하지 않습니다 — 실제 바이너리 파일 자체가 아키텍처별로 다릅니다).
